...
export async function initialize(server, client) {
    ...
    server.tool("obs-get-persistent-data", "Gets the value of a slot from the selected persistent data realm", {
    ...
    server.tool("obs-set-persistent-data", "Sets the value of a slot from the selected persistent data realm", {
        realm: z.string().describe("The data realm to select. OBS_WEBSOCKET_DATA_REALM_GLOBAL or OBS_WEBSOCKET_DATA_REALM_PROFILE"),
    ...
    server.tool("obs-get-scene-collection-list", "Gets an array of all scene collections", {}, async () => {
        try {
    ...
    server.tool("obs-set-current-scene-collection", "Switches to a scene collection", {
    ...
    server.tool("obs-create-scene-collection", "Creates a new scene collection, switching to it in the process", {
    ...
    // GetProfileList tool
    server.tool("obs-get-profile-list", "Gets an array of all profiles", {}, async () => {
        try {
    ...
    server.tool("obs-set-current-profile", "Switches to a profile", {
    ...
    // CreateProfile tool
    server.tool("obs-create-profile", "Creates a new profile, switching to it in the process", {
    ...
    // RemoveProfile tool
    server.tool("obs-remove-profile", "Removes a profile. If the current profile is chosen, it will change to a different profile first", {
    ...
    server.tool("obs-get-profile-parameter", "Gets a parameter from the current profile's configuration", {
    ...
    server.tool("obs-set-profile-parameter", "Sets the value of a parameter in the current profile's configuration", {
    ...
    server.tool("obs-get-video-settings", "Gets the current video settings", {}, async () => {
        try {
    ...
    server.tool("obs-set-video-settings", "Sets the current video settings", {
        ...
        try {
            ...
            const requestParams = {};
            if (params.fpsNumerator !== undefined)
                requestParams.fpsNumerator = params.fpsNumerator;
            if (params.fpsDenominator !== undefined)
                requestParams.fpsDenominator = params.fpsDenominator;
            if (params.baseWidth !== undefined)
                requestParams.baseWidth = params.baseWidth;
            if (params.baseHeight !== undefined)
                requestParams.baseHeight = params.baseHeight;
            if (params.outputWidth !== undefined)
                requestParams.outputWidth = params.outputWidth;
            if (params.outputHeight !== undefined)
                requestParams.outputHeight = params.outputHeight;
    ...
    server.tool("obs-get-stream-service-settings", "Gets the current stream service settings", {}, async () => {
        try {
    ...
    server.tool("obs-set-stream-service-settings", "Sets the current stream service settings", {
    ...
    server.tool("obs-get-record-directory", "Gets the current directory that the record output is set to", {}, async () => {
        try {
    ...
    server.tool("obs-set-record-directory", "Sets the current directory that the record output writes files to", {
