...
export async function initialize(server, client) {
    // GetVersion tool
    server.tool("obs-get-version", "Gets data about the current plugin and RPC version", {}, async () => {
        try {
    ...
    // GetStats tool
    server.tool("obs-get-stats", "Gets statistics about OBS, obs-websocket, and the current session", {}, async () => {
        try {
    ...
    server.tool("obs-broadcast-custom-event", "Broadcasts a CustomEvent to all WebSocket clients", {
    ...
    server.tool("obs-call-vendor-request", "Call a request registered to a vendor", {
    ...
    // GetHotkeyList tool
    server.tool("obs-get-hotkey-list", "Gets an array of all hotkey names in OBS", {}, async () => {
        try {
    ...
    server.tool("obs-trigger-hotkey-by-name", "Triggers a hotkey using its name", {
    ...
    server.tool("obs-trigger-hotkey-by-key-sequence", "Triggers a hotkey using a sequence of keys", {
    ...
    // Sleep tool
    server.tool("obs-sleep", "Sleeps for a time duration or number of frames", {
