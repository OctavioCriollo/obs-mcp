...
export async function initialize(server, client) {
    ...
    server.tool("obs-get-stream-status", "Get the current streaming status", {}, async () => {
        try {
    ...
    // StartStream tool
    server.tool("obs-start-stream", "Start streaming in OBS", {}, async () => {
        try {
    ...
    // StopStream tool
    server.tool("obs-stop-stream", "Stop streaming in OBS", {}, async () => {
        try {
    ...
    // ToggleStream tool
    server.tool("obs-toggle-stream", "Toggle the streaming state in OBS", {}, async () => {
        try {
    ...
    server.tool("obs-send-stream-caption", "Sends CEA-608 caption text over the stream output", {
